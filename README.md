...
基于 Next.js 16 构建的个人技术博客系统，采用 Glassmorphism 视觉风格，支持 Markdown/MDX 编辑、代码高亮、Mermaid 图表、KaTeX 数学公式渲染，内置独立页面、碎碎念、友情链接、阅读进度条、相关文章推荐、搜索高亮等功能。
...
- **独立页面** — 自定义 slug 的 Markdown 页面（"关于我"、"我的项目"等）
- **碎碎念/微动态** — 类似微博的短内容发布，无标题无封面，时间线浏览
- **友情链接** — 分类管理友链，卡片式网格展示，支持外链图标
...
- **阅读进度条** — 文章页顶部蓝紫渐变进度条，实时显示阅读进度
- **返回顶部** — 滚动后右下角浮现返回顶部按钮
- **相关文章推荐** — 基于标签/分类智能推荐 3 篇相关文章
- **搜索增强** — 搜索关键词高亮 + 摘要片段预览，快速定位内容
...
### 移动端适配
- **响应式布局** — 所有页面适配手机端，支持 320px-414px 屏幕宽度
- **移动端菜单** — 管理后台抽屉式侧边栏，汉堡菜单触发
- **触摸优化** — 按钮和链接增大点击区域，表单输入框适配移动端
...
│   ├── admin/                # 管理后台（仪表盘/文章/分类/标签/专栏/页面/友链/碎碎念/评论）
│   ├── api/                  # API 路由
│   │   ├── admin/            # 后台管理 API
│   │   ├── articles/         # 文章公开 API
│   │   ├── auth/             # 认证 API
...
│   ├── ReadingProgress.tsx   # 阅读进度条+返回顶部
...
│   └── schema.prisma
├── proxy.ts                  # 路由守卫（Next.js 16 proxy 约定）
├── next.config.ts
├── tailwind.config.ts
└── package.json
```
